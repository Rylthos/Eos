...
public:
public:
  Sandbox(const Eos::ApplicationDetails &details) : Eos::Application(details) {}
...
private:
  ...
  void postInit() override {
    ...
    shader.addShaderModule(VK_SHADER_STAGE_VERTEX_BIT,
                           "res/MovingSquare/Shaders/MovingSquare.vert.spv");
    shader.addShaderModule(VK_SHADER_STAGE_FRAGMENT_BIT,
                           "res/MovingSquare/Shaders/MovingSquare.frag.spv");
    ...
    void *tempBuffer;
    vmaMapMemory(Eos::GlobalData::getAllocator(), m_DataBuffer.allocation,
                 &tempBuffer);
  ...
  void draw(VkCommandBuffer cmd) override {
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            m_PipelineLayout, 0, 1, &m_DataDescriptorSet, 0,
                            nullptr);

    vkCmdDrawIndexed(cmd, static_cast<uint32_t>(m_Mesh.getIndices()->size()), 1,
                     0, 0, 0);
  }
  ...
  void updateData() {
    m_Data.model = glm::mat4(1.0f);
    m_Data.model =
        glm::translate(m_Data.model, glm::vec3(m_PosX, m_PosY, 0.0f));
  ...
  static bool keyboardEvent(const Eos::Events::KeyInputEvent *event) {
    ...
    if (event->key == EE::Key::KEY_ESCAPE &&
        event->action == EE::Action::PRESS) {
